<?php
  ...
  $queryUserData = sprintf("SELECT * FROM usuarios WHERE id = %d",
    mysqli_real_escape_string($conn_localhost, trim($_GET['userId']))
  );

  // Ejecutamos el query
  $resQueryUserData = mysqli_query($conn_localhost, $queryUserData) or trigger_error("El query para obtener los detalles del usuario loggeado falló");
  ...
  if(isset($_POST['userUpdateSent'])) {
    ...
    if(!isset($error)) {
      ...
      $queryUpdateUser = sprintf("UPDATE usuarios SET nombre = '%s', apellidos = '%s', password = '%s', telefono = '%s', rol = '%s' WHERE id = %d",
        ...
        mysqli_real_escape_string($conn_localhost, trim($_POST['apellidos'])),
        mysqli_real_escape_string($conn_localhost, trim($_POST['password'])),
        mysqli_real_escape_string($conn_localhost, trim($_POST['telefono'])),
        mysqli_real_escape_string($conn_localhost, trim($_POST['rol'])),
        mysqli_real_escape_string($conn_localhost, trim($_POST['userId']))
      );

      // Ejecutamos el query
      $resQueryUserUpdate = mysqli_query($conn_localhost, $queryUpdateUser) or trigger_error("El query de actualización de usuario falló");
